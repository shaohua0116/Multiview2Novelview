...
class Model(object):
    ...
    def build(self, is_train=True):
            ...
            with tf.variable_scope(scope, reuse=reuse) as scope:
                ...
                for i in range(num_deconv_layer):
                    ...
                    if num_deconv_layer - i <= num_scale:
                        ...
                        if not reuse: log.infov('flow_output_img: {}, pixel output: {}, '
        ...
        print('Num of prior {}'.format(num_prior))
        # Input {{{
        # =========
        input_image = []
        input_pose = []
        ...
        def build_loss(output_list, weights, loss_name, scale=False):
            log.warn('Build {} loss: len(output_list): {}'.format(
                loss_name, len(output_list)))
            total_loss = 0
            # l1 loss
            l1_loss = 0
